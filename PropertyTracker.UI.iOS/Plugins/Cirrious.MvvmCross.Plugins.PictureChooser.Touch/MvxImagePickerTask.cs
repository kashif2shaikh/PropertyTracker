...
namespace Cirrious.MvvmCross.Plugins.PictureChooser.Touch
{
          ...
          , IMvxPictureChooserTask
    {
		...
		public void TakePictureWithFrontCamera(int maxPixelDimension, int percentQuality, Action<Stream> pictureAvailable,
			Action assumeCancelled)
		{
			_picker.SourceType = UIImagePickerControllerSourceType.Camera;
			_picker.CameraDevice = UIImagePickerControllerCameraDevice.Front;
			ChoosePictureCommon(maxPixelDimension, percentQuality, pictureAvailable, assumeCancelled);
		}
		...
		public Task<Stream> TakePictureWithFrontCamera(int maxPixelDimension, int percentQuality)
		{
			var task = new TaskCompletionSource<Stream>();
			TakePictureWithFrontCamera(maxPixelDimension, percentQuality, task.SetResult, () => task.SetResult(null));
			return task.Task;
		}
