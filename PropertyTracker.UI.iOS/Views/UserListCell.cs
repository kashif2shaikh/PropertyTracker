...
namespace PropertyTracker.UI.iOS.Views
{
    public partial class UserListCell : MvxTableViewCell
    {
        ...
        {
			_imageViewLoader = new MvxImageViewLoader(() => this.PhotoImageView);

            this.DelayBind(() => {
                var set = this.CreateBindingSet<UserListCell, User>();
				set.Bind<UILabel>(FullNameLabel).To(user => user.Fullname);
				set.Bind<UILabel>(UsernameLabel).To(user => user.Username);                   
